...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 16 12:59:14 2024
...
def import_data():
    ...
    for i in range(num_files):
        ...
        sub_id[:] = i + 1
        df['id'] = sub_id
        all_data = pd.concat([all_data, df], ignore_index=True) #all_data = np.append(all_data) #all_data.append(df)
        print('File loaded: ' + file)
    all_data = all_data.dropna()##all_data = np.dropna(all_data)
