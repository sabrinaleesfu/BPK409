...
//add this method to read #length data from register address #regAddr
//because have find a way to make the original function 'readRegisters' work
//even after setting corresponding function and struct to be public -QQ
int8_t BMA400::readBma400Regs(uint8_t regAddr, uint8_t *dataArray, uint32_t length)
{
	return bma400_read_regs(regAddr, dataArray, length, &sensor);
}
